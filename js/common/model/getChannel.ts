...
export default function( type: ChannelType, position: number ): Channel {
  return type === 'sodiumIonLeakageChannel' ? new Channel( type, position ) :
         type === 'potassiumIonLeakageChannel' ? new Channel( type, position ) :

         type === 'sodiumIonVoltageGatedChannel' ? new Channel( type, position ) :
         type === 'potassiumIonVoltageGatedChannel' ? new Channel( type, position ) :
